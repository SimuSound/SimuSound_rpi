...
{
private:
  FILE * debugStream = stdout;     // ptr to stdout/stderr which itself is ptr to FILE
  unsigned int debugLevel = -1;  // lower means more important, 0 means print nothing, -1 is UINT_MAX which means print everything
...
public:
  ...
  void _debugPrint(const char* format, ...);  // always print, used internally
  void debugPrint(const char* format, ...);  // use default level
  void debugPrint(unsigned int level, const char* format, ...);  // use custom level

  void setDebugPrintStream(FILE * debugStream);
  void setDebugPrintLevel(unsigned int level);
...
public:
  ...
  ICM_20948_Status_e begin(uint8_t csPin, uint32_t SPIFreq = ICM_20948_SPI_DEFAULT_FREQ);
};
