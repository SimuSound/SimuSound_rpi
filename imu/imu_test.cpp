...
#include "ICM_20948_rpi.h" // Click here to get the library: http://librarymanager/All#SparkFun_ICM_20948_IMU
...
void setup()
{
  ...
  printf("Initialization of the sensor returned: %s\n", myICM.statusString());
  if (myICM.status != ICM_20948_Stat_Ok)
  {
    // printf("Trying again...\n");
    // usleep(500*1000);
    spiClose(myICM._spi_handle);
    gpioTerminate();
    exit(1);
  }
  else
...
void loop()
{
  ...
  {
    //SERIAL_PORT.print("Received data! Header: 0x"); // Print the header in HEX so we can see what data is arriving in the FIFO
    ...
    if ((data.header & DMP_header_bitmap_Quat9) > 0) // We have asked for orientation data so we should receive Quat9
    {
      ...
      printf("Q1: %.3f Q2: %.3f Q3: %.3f Accuracy: %f\n", q1, q2, q3, data.Quat9.Data.Accuracy);
    }
  }
