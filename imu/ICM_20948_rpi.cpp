...
void ICM_20948::setDebugPrintStream(FILE * debugStream)
{
  this->debugStream = debugStream; //Grab which port the user wants us to use for debugging
}
void ICM_20948::setDebugPrintLevel(unsigned int level)
{
  this->debugLevel = level; //Turn off extra print statements
}

// https://www.ozzu.com/cpp-tutorials/tutorial-writing-custom-printf-wrapper-function-t89166.html
void ICM_20948::_debugPrint(const char* format, ...) 
{
  va_list args;
  va_start(args, format);
  vfprintf(this->debugStream, format, args);
  va_end(args);
}

void ICM_20948::debugPrint(const char* format, ...) 
{
...
void ICM_20948::debugPrint(unsigned int level, const char* format, ...) 
{
...
ICM_20948_Status_e ICM_20948::startupDefault(bool minimal)
{
  ...
  // ICM-20689 doesn't have magnetometer
  ...
  // moved here and changed for ICM-20689
  ICM_20948_fss_t FSS;
  FSS.a = gpm16;   // (ICM_20948_ACCEL_CONFIG_FS_SEL_e)
  ...
  {
    debugPrint("ICM_20948::startupDefault: minimal startup complete!\n");
    return status;
  }
...
ICM_20948_Status_e ICM_20948::enableDMPSensor(enum inv_icm20948_sensor sensor, bool enable)
{
  if (_device._dmp_firmware_available == true) // Should we attempt to enable the sensor?
  {
    ...
    debugPrint("%d", (int)_device._enabled_Android_0);
    debugPrint("  _enabled_Android_1: ");
    debugPrint("%d", (int)_device._enabled_Android_1);
    debugPrint("  _dataOutCtl1: ");
    debugPrint("%d", (int)_device._dataOutCtl1);
    debugPrint("  _dataOutCtl2: ");
    debugPrint("%d", (int)_device._dataOutCtl2);
    debugPrint("  _dataRdyStatus: ");
    debugPrint("%d", (int)_device._dataRdyStatus);
    debugPrint("\n");
    return status;
  }
  return ICM_20948_Stat_DMPNotSupported;
...
ICM_20948_Status_e ICM_20948::enableDMPSensorInt(enum inv_icm20948_sensor sensor, bool enable)
{
  if (_device._dmp_firmware_available == true) // Should we attempt to enable the sensor interrupt?
  {
    ...
    debugPrint("%d", (int)_device._enabled_Android_intr_0);
    debugPrint("  _enabled_Android_intr_1: ");
    debugPrint("%d", (int)_device._enabled_Android_intr_1);
    debugPrint("  _dataIntrCtl: ");
    debugPrint("%d", (int)_device._dataIntrCtl);
    debugPrint("\n");
    return status;
  }
  return ICM_20948_Stat_DMPNotSupported;
...
ICM_20948_Status_e ICM_20948::setGyroSF(unsigned char div, int gyro_level)
{
  if (_device._dmp_firmware_available == true) // Should we attempt to set the Gyro SF?
  {
    ...
    debugPrint("%d", (int)_device._gyroSFpll);
    debugPrint("  Gyro SF is: ");
    debugPrint("%d", (int)_device._gyroSF);
    debugPrint("\n");
    return status;
  }
  return ICM_20948_Stat_DMPNotSupported;
...
ICM_20948_Status_e ICM_20948::startupMagnetometer(bool minimal)
{
  ...
  if (tries == MAX_MAGNETOMETER_STARTS)
  {
    ...
    debugPrint("%d", (int)MAX_MAGNETOMETER_STARTS);
    debugPrint("). Returning ICM_20948_Stat_WrongID\n");
  ...
  if (minimal) // Return now if minimal is true
  {
    debugPrint("ICM_20948::startupMagnetometer: minimal startup complete!\n");
    return status;
  }
...
ICM_20948_Status_e ICM_20948::magWhoIAm(void)
{
  ...
  debugPrint(" (should be 72) whoiam2: ");
  debugPrint("%d", (int)whoiam2);
  debugPrint(" (should be 9). Returning ICM_20948_Stat_WrongID\n");
...
ICM_20948_Status_e ICM_20948_SPI::begin(uint8_t csPin, uint32_t SPIFreq)
{
  ...
  _spi_handle= spiOpen(csPin, SPIFreq, 0);
  if(_spi_handle < 0)
  {
  ...
  char tmp[1] = {0};
  // int bytes_written = spiWrite(_spi_handle, tmp, 1);
  // if(bytes_written != 1)
  // {
  //   printf("spiWrite 1 failed\n");
  //   return ICM_20948_Stat_Err;
  // }
...
ICM_20948_Status_e ICM_20948_write_SPI(uint8_t reg, uint8_t *data, uint32_t len, void *user)
{
  ...
  int _spi_handle = ((ICM_20948_SPI *)user)->_spi_handle;
  ...
  // _spi->transfer(0x00);
  // _spi->endTransaction();
  char *tmp = (char *)malloc(len + 1); // contains reg + data
  tmp[0] = ((reg & 0x7F) | 0x00);
  ...
  // }
  // printf("\n");
  int bytes_written = spiWrite(_spi_handle, (char*)tmp, len + 1);
  free(tmp);
  if(bytes_written != len + 1)
  {
    printf("spiWrite data failed\n");
    return ICM_20948_Stat_Err;
  }
...
ICM_20948_Status_e ICM_20948_read_SPI(uint8_t reg, uint8_t *buff, uint32_t len, void *user)
{
  ...
  // _spi->transfer(0x00);
  // _spi->endTransaction();
  char *tmp = (char *)malloc(len + 1); // contains reg and then all 0's
  memset(tmp, 0, len + 1);
  tmp[0] = ((reg & 0x7F) | 0x80);

  char *tmp2 = (char *)malloc(len + 1); // will contain one useless byte plus the read data
  memset(tmp2, 0, len + 1);
  // memcpy(tmp+1, data, len);
  ...
  // }
  // printf("\n");
  int bytes_read = spiXfer(_spi_handle, tmp, tmp2, len + 1);
  // printf("spiXfer recv: ");
  // for(int i = 0; i < len + 1; ++i)
  // {
  //   printf("%.2x ", tmp2[i]);
  ...
  if(bytes_read != len + 1)
  {
    printf("spiRead data failed\n");
    free(tmp2);
    return ICM_20948_Stat_Err;
  }
  memcpy(buff, tmp2 + 1, len);
  free(tmp2);
  //   delayMicroseconds(5);
  // digitalWrite(cs, HIGH);
  // _spi->endTransaction();
